...
- `wasm_pdf_replace_text_page_object(handle, pageIndex, objectIndex, text)` replaces a text page object's contents while preserving the original PDF font handle, font size, and fill color where safe. If the replacement needs broader glyph coverage, for example non-ASCII text or new characters in a subset font, the replacement switches to a bundled embedded TrueType fallback with a similar sans/serif/mono family. Newline characters create separate text line objects using the same style and a default `1.2 * fontSize` line height. It regenerates page content and returns `1` on success or `0` on failure.
