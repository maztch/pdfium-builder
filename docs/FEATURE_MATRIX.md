...
| Update annotation border | `wasm_pdf_set_annotation_border` | `updateAnnotation` with `borderWidth` | No | [API](API.md#annotations) | Stable | Direct API exposes `setAnnotationBorderWidth`; worker coverage can be added with annotation editing endpoints. |
